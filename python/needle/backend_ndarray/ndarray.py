...
class BackendDevice:
    ...
    def full(self, shape: tuple[int, ...], fill_value: float, device, dtype: str = "float32") -> "NDArray":
        dtype = "float32" if dtype is None else dtype
        arr = self.empty(shape, device, dtype=dtype)
        arr.fill(fill_value)
        return arr
...
def full(shape: tuple[int, ...], fill_value: float, dtype: str = "float32", device: BackendDevice | None = None) -> NDArray:
    device = device if device is not None else default_device()
    return device.full(shape, fill_value, device, dtype=dtype)
